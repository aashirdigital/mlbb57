...
const walletPaymentModel = require("../models/walletPaymentModel");
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const mode = await walletPaymentModel.findOne({});
      if (!mode) {
        const newMode = new walletPaymentModel({
      ...
      const updateMode = await walletPaymentModel.findOneAndUpdate(
        {},
        {
          $set: {
            paymentMode: req.body.paymentMode,
